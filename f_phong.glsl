...
void main(void) {
    ...
    float rv1 = pow(clamp(dot(r1, nV), 0.0, 1.0), 25.0); //sila odbicia dla swiatla 1
    float rv2 = pow(clamp(dot(r2, nV), 0.0, 1.0), 25.0); //sila odbicia dla swiatla 2

    vec4 kd = texture(textureMap, iTexCoord);
	vec4 ks = vec4(0.7f, 0.7f, 0.1f, 0.6f);
