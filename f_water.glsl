...
void main(void) {
    ...
    float rv1 = pow(clamp(dot(r1, nV), 0.0, 1.0), 2.0); //sila odbicia dla swiatla 1
    float rv2 = pow(clamp(dot(r2, nV), 0.0, 1.0), 2.0); //sila odbicia dla swiatla 2

    vec4 kd = texture(textureMap, iTexCoord) * vec4(1.0f, 1.0f, 0.0f, 0.1f);
	vec4 ks = vec4(0.8f, 0.8f, 0.1f, 0.1f);

    pixelColor = vec4(ml1 * kd.rgb, ks.a) + vec4(ml2 * kd.rgb, ks.a) + vec4(ks.rgb*rv1, 0) + vec4(ks.rgb*rv2, 0);
}